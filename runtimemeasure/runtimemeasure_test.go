...
// The anchor bytes behind the hardware-confirmed vectors below. That run used
// a PKIX public-key PEM as its anchor, trailing newline included; the package
// hashes whatever bytes it is given, so the shape matters only in that these
// exact bytes produced the pinned registers.
const testAnchor = `-----BEGIN PUBLIC KEY-----
...
func TestConventionGoldenVectors(t *testing.T) {
	for _, tc := range []struct {
		...
		// Hardware-confirmed: RTMR[3] as read back from a TDX CVM launched
		// with exactly these anchor bytes.
		{"Seed(testAnchor)", Seed([]byte(testAnchor)),
			"a9b91d920971de864899fb5925c4b5230bf88750dd59866d8d34aeb975e86761ea7488ade961908d9595b6202c9e6470"},
		{"FromDigestsSeeded(Seed(testAnchor),[A,B])",
			FromDigestsSeeded(Seed([]byte(testAnchor)), []string{digestA, digestB}),
...
// Seed must be exactly one extend of the anchor's SHA-384, so a verifier can
// chain per-workload extends onto it.
func TestSeedIsSingleExtendOfAnchorDigest(t *testing.T) {
	anchor := []byte("some anchor bytes\n")
	if Seed(anchor) != Extend(Zero, sha512.Sum384(anchor)) {
		t.Error("Seed must equal Extend(Zero, SHA384(anchor))")
	}
}

// The seed is over the EXACT bytes supplied: a re-wrapped or newline-stripped
// input is a different anchor as far as the register is concerned. Pinning it
// here makes the footgun a deliberate choice rather than a surprise.
func TestSeedIsByteExact(t *testing.T) {
	withNewline := []byte("anchor bytes\n")
	withoutNewline := withNewline[:len(withNewline)-1]
	if Seed(withNewline) == Seed(withoutNewline) {
		t.Error("Seed must not normalize its input; a trailing newline changes the seed")
	}
}

// TestSeedMatchesHardware pins the seed extend to the exact value a hardware
// run produced: anchorDigest is SHA-384(anchor) from that run, wantRTMR3 what
// /sys/.../rtmr3:sha384 read back after launch.
func TestSeedMatchesHardware(t *testing.T) {
	const (
		anchorDigest = "0c06aa4f364e480ece13c58b1585dab43d7222fa331ccc9ff05ea18fdd39a4d9d75e87d711ac6aeda2782c2e339de7c1"
		wantRTMR3    = "db479dfe6333f8d3a2761494b6004bc4332688c6d5b72577b48ecfc0409e4cb53988dcd26b89ec605a81b00e7f0e0863"
	)
	dig, err := hex.DecodeString(anchorDigest)
	if err != nil {
		t.Fatal(err)
	}
	// Seed hashes the anchor; here we already have the digest, so pin the
	// second step (the Zero extend) directly.
	got := Extend(Zero, [Size]byte(dig))
	if hex.EncodeToString(got[:]) != wantRTMR3 {
		t.Errorf("RTMR[3] from anchor digest = %x, want %s (hardware-confirmed)", got, wantRTMR3)
	}
}
...
// The whole point of the seed: on an anchor-bound guest, per-workload extends
// chain onto it, so a verifier that starts from Zero gets the wrong answer.
func TestFromDigestsSeededChainsOntoSeed(t *testing.T) {
	seed := Seed([]byte("anchor bytes\n"))
...
// TestHostDataGolden pins the SNP binding formula: a changed
// vector here breaks the launcher, the guest initrd, and every verifier at
// once, exactly like the RTMR[3] vectors above.
func TestHostDataGolden(t *testing.T) {
	anchor := []byte("-----BEGIN PUBLIC KEY-----\nAAAA\n-----END PUBLIC KEY-----\n")
	const want = "f61b347fbb8e6469f294c1faa146d37688e5c1c8a6b1b25653ee85ad10b7447a"
	if got := hex.EncodeToString(hostDataSlice(HostData(anchor))); got != want {
		t.Errorf("HostData = %s, want %s", got, want)
	}
}

// Same exact-bytes rule as Seed: a trailing newline changes the binding.
func TestHostDataIsByteExact(t *testing.T) {
	withNewline := []byte("anchor bytes\n")
	withoutNewline := withNewline[:len(withNewline)-1]
	if HostData(withNewline) == HostData(withoutNewline) {
		t.Error("HostData must not normalize its input; a trailing newline changes the binding")
	}
}
