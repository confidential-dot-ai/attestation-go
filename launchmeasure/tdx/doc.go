// Package tdx predicts the Intel TDX build-time measurement (MRTD) a guest will
// report, computed offline from the TDVF firmware image before the guest is
// ever started. It is the TDX counterpart to package snp.
//
...
// guest shape booting the same TDVF; this was confirmed against two live TDX
// guests of different vCPU shapes. The asymmetry with SEV-SNP is the reason
// these are separate packages: an SNP launch digest is per guest shape, an
// MRTD is per firmware build.
//
